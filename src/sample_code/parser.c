...
void move_cursor_to_next_char_occurence(
  ...
  do {
    ...
    if (c == EOF) {
      log_event(
        ET_ERROR,
        "Failed to parse: expected '%s'", searchChar
      );
...
void parse_struct_field(
  ...
  if (fscanf(file, format, destination) != 1) {
    if (simName != NULL) {
      log_event(
        ET_ERROR, "Failed to parse simulation '%s': unable to read %s",
        simName, fieldName
      );
    } else {
      log_event(
        ET_ERROR, "Failed to parse simulation: unable to read %s", fieldName
      );
    }
...
void parse_simulation_max_iterations(
  FILE *const file,
